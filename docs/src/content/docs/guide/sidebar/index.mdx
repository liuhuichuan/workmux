---
title: "Sidebar"
description: A persistent agent status sidebar for tmux windows and WezTerm tabs
---
...
top edge of every tmux window -- or every WezTerm tab on Windows. Unlike the
dashboard, which is a full-screen TUI you open on demand, the sidebar stays on
...
configured, and one of the two backends it runs under: tmux, or WezTerm on
Windows.
:::
...
On Windows the sidebar is scoped to one WezTerm workspace and appears in every
tab of it, so the global and `--session` forms are the same command.
...
well. It uses a daemon + client architecture with event-driven rendering. Where
WezTerm is the backend the shape is a different one -- each sidebar pane polls
WezTerm and the state store itself, and there is no daemon to push to. That is
written up under [WezTerm](/guide/wezterm/):
