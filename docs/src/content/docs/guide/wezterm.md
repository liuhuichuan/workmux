...
- A listing that does not come back leaves a silence behind it: a mux that has stopped answering is left alone, the wait doubling with each unanswered call up to 30 seconds, and one answered call puts it back on its poll. A sidebar's poll reports "cannot read the panes" at that cadence instead of spending the 20-second deadline on a wedged GUI once a second, and stops adding processes to the load that wedged it
- A mux that has stopped answering does not cost a merge or a removal its cleanup: the worktree and branch are removed by a deferred worker, which reads a mux that cannot report a window as a window that went down with it. Only the source window close is skipped, and the merge reports success rather than a cleanup failure
